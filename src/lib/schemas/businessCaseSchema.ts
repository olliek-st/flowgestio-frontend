// src/lib/schemas/businessCaseSchema.ts
import { z } from "zod";
...
// Local type alias for the LineItemCategory union — avoids duplicating the enum values.
// TypeScript infers string-array literals as string[] without an explicit annotation.
type _Cat = z.infer<typeof zLineItemCategory>;

export const DEFAULT_ORGANIZATIONAL_SETTINGS = {
  requiresRegulatory: false,
  // `as const` narrows literal strings to the Zod union types
  defaultCurrency: "CAD" as const,
  defaultDiscountRate: 8,
  npvThreshold: 50000,
  irrThreshold: 50000,
  // `as string[]` widens never[] (inferred for empty literal arrays in strict mode)
  mandatoryApprovers: [] as string[],
  ...
  // `as _Cat[]` narrows string[] to the LineItemCategory union required by Zod
  baselineRequirements: {
    new_development: {
      requiredCategories: ["maintenance", "opex", "risk_avoidance"] as _Cat[],
      description: "New projects must show ongoing operational costs and avoided risks"
    },
    infrastructure: {
      requiredCategories: ["maintenance", "opex", "compliance"] as _Cat[],
      description: "Infrastructure projects must include maintenance and compliance costs"
    },
    process_improvement: {
      requiredCategories: ["opex", "productivity"] as _Cat[],
      description: "Process improvements must show current operational inefficiencies"
    },
    compliance: {
      requiredCategories: ["compliance", "risk_avoidance"] as _Cat[],
      description: "Compliance projects must show regulatory costs and avoided penalties"
    },
    maintenance: {
      requiredCategories: ["maintenance", "risk_avoidance"] as _Cat[],
      description: "Maintenance projects must show current maintenance burden and failure risks"
    }
  },
  financeRatePct: 8,
  reinvestRatePct: 8,
  defaultRecurrenceEnd: "horizon" as const,
};
// Convenience alias — importers can use either name
export const businessCaseSchema = businessCaseStep3Schema;
